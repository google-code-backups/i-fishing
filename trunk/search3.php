...
$var = mysql_real_escape_string(@$_REQUEST['searchterm']);
...
$query = "SELECT * , MATCH (NAMA_DEPAN, NAMA_BELAKANG) AGAINST ('".$trimm."') AS ID_MEMBER FROM akun_pengguna WHERE MATCH (NAMA_DEPAN, NAMA_BELAKANG) AGAINST ('+".$trimm."') ORDER BY ID_MEMBER DESC";
 ...
 if($row_num_links_main < 1){
    $query = "SELECT * FROM akun_pengguna WHERE NAMA_DEPAN LIKE '%$trimm%' OR NAMA_BELAKANG LIKE '%$trimm%'  ORDER BY ID_MEMBER DESC";
    $numresults=mysql_query ($query);
...
echo '<form action="search3.php" method="get">
        <div>
        <input name="q" type="text" value="'.$q.'">
        <input name="searchterm" type="submit" value="Search">
        </div>
</form>';
...
if( isset ($resultmsg)){
    ...
    $query_value = "SELECT * FROM akun_pengguna WHERE field_id = '".$value."'";
